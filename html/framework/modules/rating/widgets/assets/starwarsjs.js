(function ($) {
    ...
    $.fn.starwarsjs = function (options) {
        var settings = $.extend({
            target: this.selector, /* .selector is deprecated, please define target in options on initialization */
            stars: 1,
            range: [],
            count: 1,
            disable: -1,
        ...
        if (starwarsjs.stars) {
            ...
            $(starwarsjs.target + ' .' + starwarsjs.star).each(function (key, value) {
                ...
                $(this).hover( // change star's color after mouse hover
                    function () {
                        //Note: andSelf() This function has been deprecated and is now an alias for .addBack(), which should be used with jQuery 1.8 and later.
                        //mp $(this).prevAll().andSelf().addClass('over');
                        $(this).prevAll().addBack().addClass('over');
                    },
                    function () {
                        //Note: andSelf() This function has been deprecated and is now an alias for .addBack(), which should be used with jQuery 1.8 and later.
                        //mp $(this).prevAll().andSelf().removeClass('over');
                        $(this).prevAll().addBack().removeClass('over');
                    }
                );
                ...
                $(this).on('click', function () { // get rate after click
                    ...
                    //Note: andSelf() This function has been deprecated and is now an alias for .addBack(), which should be used with jQuery 1.8 and later.
                    //mp $(this).prevAll().andSelf().addClass(starwarsjs.checked_class);
                    $(this).prevAll().addBack().addClass(starwarsjs.checked_class);
